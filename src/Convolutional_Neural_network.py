...
import random
import sys
np.set_printoptions(threshold='nan') # This will show all the array in numpy - for testing purposes

def Sequence_extraction(percentage):
    promoter = "output/experimental_promoter_sequence.tsv"
    non_promoter = "output/non_promoter_sequence.tsv"
    ...
    with open(promoter, 'rb') as csvfile: #open promoter data
    ...
    percentage = float(percentage)/100 #The input percentage will determine your percentage of training sample number from the available provided sample (Usually positive sample)
    ...
    with open(non_promoter, 'rb') as csvfile: #open non promoter data
        reader = csv.reader(csvfile, delimiter='\t', quotechar='|', quoting=csv.QUOTE_MINIMAL)#csv reader tab separate
        for line in reader:
...
def sequence_extraction_real(complement_list):
...
def ANN_model(X, y, seq_length, pos_tensor, neg_tensor, Test_pos_tensor, Test_neg_tensor, complement_tensor, name):
    ...
    hist = model.fit(X, y, validation_split=0.3, nb_epoch=5)  # starts training
...
def main(percentage, fasta_file):
    X, y, seq_length, pos_tensor, neg_tensor, Test_pos_tensor, Test_neg_tensor = Sequence_extraction(percentage)
    fasta = fasta_file
    complement_list, name = fasta_sequence(fasta)
    complement_tensor= sequence_extraction_real(complement_list)
    
    ANN_model(X, y, seq_length, pos_tensor, neg_tensor, Test_pos_tensor, Test_neg_tensor, complement_tensor, name)
